...
public class AccountController {
    ...
    @GetMapping
    public String handleGet(@RequestParam(name = "size", defaultValue = "10") int size, Model model) {
        List<Account> accountList = Stream.generate(dataGenerator::generateAccount).limit(size).collect(Collectors.toList());
